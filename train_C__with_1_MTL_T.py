...
parser.add_argument('--MTL_T_type', type = str, help = 'load MTL T', default='T1')
parser.add_argument('--T_lr', type=float, help='learning rate for training MTL T')
...
model_save_path = './checkpoint/C*_from_MTL_' + args.MTL_T_type + '_Tlr_' + str(args.T_lr) + '.pth'
T_path = './checkpoint/MTL_' + args.MTL_T_type + '_Tlr_' + str(args.T_lr) + '.pth'
clean_path = '/content/drive/MyDrive/final_retrain_21_mar/checkpoint/best_clean_classifier_anneal_seed_5.pth'
...
# load C
C_state = torch.load(clean_path)
net_T.load_state_dict(C_state['net'])

# load T
T_state = torch.load(T_path)
T_weight = T_state['net']
new=list(T_weight.items())
layer_name, weights = new[0]

# update linear_t layer weight
net_T_state_dict = net_T.state_dict()
for k, v in net_T_state_dict.items():
  if k == 'module.linear_t.weight':
    net_T_state_dict[k] = weights

net_T.load_state_dict(net_T_state_dict)
...
result_path = './result/C*_from_MTL_' + args.MTL_T_type + '_Tlr_' + str(args.T_lr) + '.txt' 
