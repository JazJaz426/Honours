...
model_save_path = './checkpoint/robust_classifier_T2_T3_seed_5.pth'
...
# Training
def train(epoch, T1_train_all, T2_train_all):
    print('\nEpoch: %d' % epoch)
    net.train()
    train_loss = 0
    correct = 0
    ...
    for batch_idx, (inputs, targets) in enumerate(train_loader):
        ...
        loss_1 = criterion(torch.log(noisy_posterior_reshaped_1), targets_1)
        loss_2 = criterion(torch.log(noisy_posterior_reshaped_2), targets_2)
...
def validation(epoch, T1_val_all, T2_val_all):
    global best_val_acc
    net.eval()
    val_loss = 0
    correct = 0
...
for epoch in range(start_epoch, start_epoch + args.n_epoch):
    train(epoch, T2_train_all, T3_train_all)
    validation(epoch, T2_val_all, T3_val_all)
    test_per_epoch(test_loader, net)
    scheduler.step()

# Testing
val_acc, test_acc = test_final(model_save_path, test_loader)
print(f'Validation AccuraT2_train_allcy : {val_acc}%\n')
print(f'Testing Accuracy : {test_acc}%\n')

result_path = './result/train_T2_T3_together_seed_5.txt' 
