...
parser.add_argument('--T_trained_lr', default=3e-4, type=float, help='learning rate used for the trained T')
parser.add_argument('--T_fine_tune_lr', default=5e-4, type=float, help='learning rate for finetuning T')
parser.add_argument('--C_lr', default=3e-4, type=float, help='learning rate for training C* from scratch')
parser.add_argument('--lr', default=3e-4, type=float, help='learning rate for overall')
...
# the 3 trained Ts for loading
trained_T1_path = '/content/drive/MyDrive/final_retrain_21_mar/checkpoint/MTL_T1_Tlr_' + str(args.T_trained_lr) + '.pth'
trained_T2_path = '/content/drive/MyDrive/final_retrain_21_mar/checkpoint/MTL_T2_Tlr_' + str(args.T_trained_lr) + '.pth'
trained_T3_path = '/content/drive/MyDrive/final_retrain_21_mar/checkpoint/MTL_T3_Tlr_' + str(args.T_trained_lr) + '.pth'
trained_T_paths = [trained_T1_path, trained_T2_path, trained_T3_path]

# save path for 3 fine-tuned T
fine_tuned_T1_save_path = '/content/drive/MyDrive/final_retrain_21_mar/checkpoint/fine_tuned_T1_Ttrainedlr_' + str(args.T_trained_lr) + '_Tfinetunedlr_' + str(args.T_fine_tune_lr) + '_C_lr_' + str(args.C_lr) + '.pth'
fine_tuned_T2_save_path = '/content/drive/MyDrive/final_retrain_21_mar/checkpoint/fine_tuned_T2_Ttrainedlr_' + str(args.T_trained_lr) + '_Tfinetunedlr_' + str(args.T_fine_tune_lr) + '_C_lr_' + str(args.C_lr) + '.pth'
fine_tuned_T3_save_path = '/content/drive/MyDrive/final_retrain_21_mar/checkpoint/fine_tuned_T3_Ttrainedlr_' + str(args.T_trained_lr) + '_Tfinetunedlr_' + str(args.T_fine_tune_lr) + '_C_lr_' + str(args.C_lr) + '.pth'

# save path for MTL learned C*
MTL_clean_net_save_path = './checkpoint/MTL_train_C_finetune_T_Ttrainedlr_' + str(args.T_trained_lr) + '_Tfinetunedlr_' + str(args.T_fine_tune_lr) + '_C_lr_' + str(args.C_lr) + '.pth'

viz_path = 'MTL_train_C_finetune_T_Ttrainedlr_' + str(args.T_trained_lr) + '_Tfinetunedlr_' + str(args.T_fine_tune_lr) + '_C_lr_' + str(args.C_lr)
num_tasks = 3
...
def load_T_to_layer(layers_specific, T_paths):
    for i in range(3):
      ...
      T_weight = T_state['net']
      layers_specific[i].load_state_dict(T_weight)
...
def validation(epoch):
    ...
    if val_acc > best_val_acc:
        ...
        torch.save(state_T1, fine_tuned_T1_save_path) 
        torch.save(state_T2, fine_tuned_T2_save_path) 
        torch.save(state_T3, fine_tuned_T3_save_path) 
        best_val_acc = val_acc

def test_per_epoch(test_loader, model_clean, epoch):
    ...
    writer.add_scalar("Test Acc", round(float(acc)*100/total, 2), epoch)
...
load_T_to_layer(layers_specific, trained_T_paths)
# load_C(net_shared, clean_path)

criterion = nn.NLLLoss() #compare logsoftmax probability and target

parameter_dict = [{"params":net_shared.module.parameters(), "lr":args.C_lr}]
parameter_dict += [{"params":layers_specific[i].module.parameters(), "lr":args.T_fine_tune_lr} for i in range(num_tasks)] 
...
if args.resume:
    ...
    net_shared_state = torch.load(MTL_clean_net_save_path)
    # load the saved states
    net_shared.load_state_dict(net_shared_state['net']) # state_dict = parameters
    optimizer.load_state_dict(net_shared_state['optimizer']) # same optimizer and scheduler for both 
    scheduler.load_state_dict(net_shared_state['scheduler'])
    start_epoch = net_shared_state['epoch'] + 1 # epoch that ended training

    T1_state = torch.load(fine_tuned_T1_save_path)
    T2_state = torch.load(fine_tuned_T2_save_path)
    T3_state = torch.load(fine_tuned_T3_save_path)
    # load the saved states
    layers_specific[0].load_state_dict(T1_state['net'])
    layers_specific[1].load_state_dict(T2_state['net'])
    layers_specific[2].load_state_dict(T3_state['net'])
...
for epoch in range(start_epoch, start_epoch + args.n_epoch):
    ...
    validation(epoch)
    test_per_epoch(test_loader, net_shared, epoch)
    scheduler.step()
